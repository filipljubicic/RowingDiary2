class ErgosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ergo.save
        ...
        format.json { render :show, status: :created, location: @ergo }
      else
  ...
  def update
    respond_to do |format|
      if @ergo.update(ergo_params)
        ...
        format.json { render :show, status: :ok, location: @ergo }
      else
  ...
  def destroy
    @ergo.destroy
    respond_to do |format|
      format.html { redirect_to ergos_url, notice: 'Ergo was successfully destroyed.' }
  ...
  private
    ...
    def ergo_params
      params.require(:ergo).permit(:description, :split, :time, :rate, :distance, :hr, :notes)
    end
end
